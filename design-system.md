...
**Активная ссылка:** инлайн-атрибут `style="color:var(--ink)"` (класса `.active` нет — **несоответствие №2**)
...
- Секция `--primary`: основные ссылки (Курсы / Кейсы / Заметки / Подход / Карьера / Менторство)
- Секция `--secondary`: вспомогательные (Инструменты / База знаний / Симуляторы / ...)
