...
### 3.31 Врезка с симулятором (`.sim-embed`)

**Где:** статьи в `/workspace/articles/`. Первое применение — `ab-test-lies`, ошибка 6 «Один критерий на все метрики».

**Назначение:** встроить интерактивный симулятор в текст, чтобы ошибку можно было воспроизвести, а не только прочитать. Закрывает пункт «интерактивные embed-блоки» из раздела 7.3.
...
<figure class="sim-embed sim-embed--classifier">
  <figcaption class="sim-embed-head">
    <div>
      ...
      <div class="sim-embed-note">12 готовых метрик из курса по A/B-статистике</div>
    </div>
...
**CSS:**
- `.sim-embed`: сплошная рамка `var(--ink)` — граница между статьёй и «прибором» (внутри iframe чужие токены: скругления, тени)
- `.prose .sim-embed`: `margin: 40px -48px` — выход из колонки текста на ширину `.article`. Селектор с `.prose` обязателен: `.prose figure` задаёт `margin: 32px 0` и по специфичности (0,0,1,1) бьёт одиночный класс
- `.sim-embed-frame`: `height: var(--sim-h)`, `border: 0`, `display: block`
- Ниже 900px: фрейм скрыт, показывается `.sim-embed-fallback` ссылкой, отрицательные поля гасятся

**Правило высоты — замерять, а не назначать.** Высота живёт в модификаторе на каждый симулятор (`--sim-h`), потому что общего числа тут быть не может: у каждого своя сетка и свой внутренний брейкпоинт — 800px у классификатора и карты, 860px у playground распределений. Занизить — значит молча обрезать читателю блок предупреждений, ради которого врезка и стоит.
...
У симуляторов без селекта сценариев (кластерный) «пресеты» заменяются сеткой параметров: перебрать крайние значения каждого ползунка и каждый вариант селекта. Для кластерного это 24 конфигурации (кластеров 4/15/60 × разброс low/high × ICC 0/0.03 × эффект 0/30%).

**Когда симулятор во врезку не годится.** Критерий: врезка оправдана, когда симулятор *показывает* механизм ошибки, а не *отвечает на запрос*. Два случая, когда он не прошёл:

*`test_selection_map`, ошибка 6.* Замерен (688–1103 по пресетам, до 1442 при всех флагах): при таком разбросе фиксированная высота либо режет вывод, либо держит 400px пустоты на коротких сценариях. Но решило не это, а суть — лукап «подбери тест под мой случай». Его открывают, когда эксперимент уже на руках, а не читая статью подряд. Ушёл ссылкой в чек-лист.

*`metric_classifier`, ошибка 12 («Метрику не завести»).* Тот же симулятор, что стоит врезкой в ошибке 6, для двенадцатой не подошёл. Механизм двенадцатой — суррогат растёт, пока настоящая цель стоит или падает. Классификатор на подходящих пресетах (`ARPPU`, «Конверсия среди увидевших фичу») выдаёт другое: `Conditional` + предупреждение о selection bias. Семья одна — «померили не то», — но поломка разная, и главное: у него нет понятий «цель» и «прокси», вывести расхождение «CTR +5%, выручка −3.2%» ему нечем. Для ошибки 6 он показывает механизм (переключение единицы наблюдения меняет требуемый критерий), для ошибки 12 — отвечает на запрос («какого типа ARPPU?»).

**Отсюда главное: критерий применяется к паре «симулятор + механизм», а не к симулятору.** Один и тот же инструмент проходит для одной ошибки и не проходит для другой. Проверять нужно каждую пару заново, а не считать симулятор «одобренным» после первой удачной врезки. «Почти по теме» хуже, чем пусто: врезка рядом с чужим механизмом уводит читателя от тезиса раздела.

Побочное следствие для повторных врезок: `metric_classifier` не поддерживает URL-параметров (ни `location.search`, ни `URLSearchParams`) и всегда стартует в пустом состоянии. Второй экземпляр того же симулятора в одной статье выглядел бы неотличимо от первого — нужный пресет читателю пришлось бы искать по подводке.

**Как мерить.** Загрузить симулятор во врезку, временно задать фрейму маленькую высоту (200px) и снять `documentElement.scrollHeight` на каждом пресете — это истинная высота контента. При большом вьюпорте метод не работает: у симуляторов `body { min-height: 100% }`, и `scrollHeight` всегда равен вьюпорту.

**Покрываем пресеты, не ручные крайности.** Все флаги разом или ползунки на максимум дают ещё +300…340px и прокручиваются внутри фрейма. До такой конфигурации читатель доходит осознанно, а пресет ему предлагает статья.

**Цена фиксированной высоты** — пустота на коротких пресетах: 148px у классификатора, 294px у распределений, 422px у карты выбора. У карты разброс самый большой (688–1103), поэтому и пустоты больше всего.

**Ограничение:** `display: none` на мобильном **не отменяет загрузку** iframe — документ скачивается (для `metric_classifier` это 25 КБ) и не показывается. Осознанный размен: убрать загрузку можно только сняв `src` и подставляя его из `matchMedia`, то есть добавив JS в статью.
...
Остаётся открытым: рендеринг `<math>` / LaTeX, Sidebar/ToC. Интерактивные embed-блоки закрыты компонентом `.sim-embed` (раздел 3.31).
