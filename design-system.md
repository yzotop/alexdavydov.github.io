...
**Когда симулятор во врезку не годится.** `test_selection_map` замерен (688–1103 по пресетам, до 1442 при всех флагах) и намеренно оставлен ссылкой: при таком разбросе фиксированная высота либо режет вывод, либо держит 400px пустоты на коротких сценариях. Плюс по сути это лукап «подбери тест под мой случай» — его открывают, когда эксперимент уже на руках, а не читая статью подряд. Критерий: врезка оправдана, когда симулятор *показывает* эффект, а не *отвечает на запрос*.
