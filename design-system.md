...
- `.sim-embed-frame`: фиксированная высота, `border: 0`, `display: block`
- Ниже 900px: фрейм скрыт, показывается `.sim-embed-fallback` ссылкой, отрицательные поля гасятся

**Правило высоты — замерять, а не назначать.** Высота ставится по самому высокому состоянию конкретного симулятора, прогнав все его пресеты. У каждого симулятора своя геометрия и свой внутренний брейкпоинт; переносить чужое число нельзя. Для `metric_classifier` это 880px (максимум 869px на пресетах AOV / время доставки / RPM). Занизить — значит молча обрезать читателю блок предупреждений, ради которого врезка и стоит.
