...
---## Term links (Glossary references)### Цель
Термины и метрики в проекте используются как **точки углубления**, а не как визуальные маркеры.
Основной текст должен читаться линейно, без постоянных отвлекающих акцентов.### Правило отображения
...
- Пользователь сам решает, когда углубляться в определение### UX-паттерн
Используется принцип *progressive disclosure*:
> сложность раскрывается только по запросу пользователя### Техническая реализация
...
- Определения **не дублируются** в контенте страниц### Запрещено
